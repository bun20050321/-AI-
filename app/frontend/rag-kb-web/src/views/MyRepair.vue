  ...
  <div class="repair-page">
    ...
    <div class="query-area">
      <div class="query-row">
        <div class="query-item">
          <label>工单编号</label>
          <input v-model="queryForm.id" type="text" class="query-input" placeholder="请输入工单编号" />
        </div>
        <div class="query-item">
          <label>组号 (group_code)</label>
          <input v-model="queryForm.groupCode" type="text" class="query-input" placeholder="请输入组号" />
        </div>
        <div class="query-item">
          <label>设备名称</label>
          <input v-model="queryForm.equipmentName" type="text" class="query-input" placeholder="请输入设备名称" />
        </div>
        <div class="query-item">
          <label>故障标题</label>
          <input v-model="queryForm.faultTitle" type="text" class="query-input" placeholder="请输入故障标题" />
        </div>
      </div>
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="item in filteredList" :key="item.id">
            ...
            <td><span :class="['status-badge', statusClass(item.handleResult)]">{{ statusText(item.handleResult) }}</span></td>
            <td>
              <button class="btn-link" @click="openDetail(item)">查看</button>
              <button class="btn-link btn-danger" @click="deleteOrder(item.id)">删除</button>
            </td>
          </tr>
    ...
    <div class="modal-overlay" :class="{ active: showAddModal }" @click.self="closeAddModal">
      <div class="modal" style="width: min(560px, calc(100% - 32px));">
        <div class="modal-header"><h2>&#10133; 新增工单</h2><button class="modal-close" @click="closeAddModal">&times;</button></div>
        ...
        <div class="modal-body" v-if="currentItem">
          <div class="detail-section">
            ...
            <div class="detail-grid">
              ...
              <div class="detail-item"><label>状态</label><span :class="['status-badge', statusClass(currentItem.handleResult)]">{{ statusText(currentItem.handleResult) }}</span></div>
            </div>
          </div>
          <div class="detail-section">
            <h4>故障详情</h4>
...
// ===== 过滤后的列表（输入即搜索，基于 queryForm 实时过滤）=====
...
// ===== 查询 =====
function doQuery() {
  // 输入即搜索已自动过滤，点击查询只是给用户一个反馈
  const count = filteredList.value.length
  alert('查询完成，共 ' + count + ' 条结果')
}
function resetQuery() {
...
.query-row { display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; margin-bottom: 12px; }
...
.btn-link.btn-danger { color: var(--error); border-color: #fecaca; margin-left: 4px; }
.btn-link.btn-danger:hover { background: #fee2e2; color: var(--error); }
