...
async function submitRepair() {
  ...
  try {
    ...
    // 根据电器类型映射组号，确保搜索时groupCode有值
    const groupCodeMap = { air: 'G-AIR', washer: 'G-WASH', fridge: 'G-FRIDGE', rice: 'G-RICE', ac: 'G-AC', microwave: 'G-MW' }
    const newAppt = { id: Date.now(), ...data, equipmentName: data.applianceModel, deviceName: data.applianceModel, faultTitle: data.faultDescription?.substring(0, 30) || '', faultPhenomenon: data.faultDescription, faultCause: '', handleMethod: '', handleResult: '待处理', repairPerson: data.userName, repairTime: (data.preferredDate || '') + ' ' + (data.preferredTime === 'morning' ? '上午' : data.preferredTime === 'afternoon' ? '下午' : data.preferredTime === 'evening' ? '晚上' : ''), status: 'pending', groupCode: groupCodeMap[data.applianceId] || 'G-OTHER', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }
